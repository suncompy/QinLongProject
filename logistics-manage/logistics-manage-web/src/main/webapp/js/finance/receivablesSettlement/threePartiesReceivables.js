...
(function($) {
  ...
  $("#btnCustomerAccountsSave").click(function(e){
	...
	function accMul(arg1,arg2){   
  	...
  	$("#projectId").change(function(e) {
		...
		var type = "GET";
		var url = "api/getInvoiceManagementByProjectId.do?projectId="+id+"&invoiceStatus=0";
		$.fd.ajax({
			url : url,
			type : type,
			dataType :"json",
			...
			success : function(data) {
				  ...
				  if(data.code == 200){
					  $.each(data.data.data,function(index,customerPack){
						  selectStr += '<option value="' + customerPack.invoiceId + '">' + customerPack.invoiceId + '</option>';
					  })
				  }
				  // 填充下拉列表
  	...
  	$("#customerPackId").change(function(e) {
		...
		var type = "GET";
		var url = "api/getInvoiceManagementByProjectId?projectId="+projectId+"&invoiceId="+id+"&invoiceStatus=0";
		$.fd.ajax({
			url : url,
			type : type,
			dataType :"json",
 ...
 //收款账户改变
	...
	$("#btnCustomerAccountsHistoryDetail").click(function(e) {
	    e.preventDefault();
	    window.open("threePartiesReceivablesHistory.do", "contentIframe");  
  });
