	    ...
	    return function(){
	      iHtml = "<tr class='newTr'><td><label class='demo--label'>\
	          ...
	          <span class='demo--checkboxInput'></span>\
	                  </label></td><td id="+ (count++) + ">"+ count +"</td><td title='双击可编辑'></td><td title='双击可编辑' onkeyup='placeHtml(this)'>~</td>" +
...
//规格序列号递增加一行
   var adds = (function(start){  
    ...
    var iHtmls = '';  
    return function(){
      iHtmls = "<tr class='newTr'><td id="+ (counts++) + ">"+ counts +"</td><td title='双击可编辑'></td><td style='text-align:center;'><button type=button style='padding: 8px; border: 1px solid #dde3ef;' onclick='$(this).parent().parent().remove()'>删除<button></td></tr>";
	  ...
	  $("body").on("click",".moreAdds",adds);
		...
		$("#addCargo").click(function() {
			...
			$("#addCargoName").attr("readOnly",false); 
			$("#tPointBod").empty();
			$("#tSpecBod").empty();
			var add = (function(start){  
			     count = 0;
