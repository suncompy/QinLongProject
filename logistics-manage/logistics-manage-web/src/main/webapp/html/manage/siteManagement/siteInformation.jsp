...
<html>
	...
	<head>
		 ...
		 $(function(){
			//新建站点
			   $("#addSite").click(function(){
			   	...
			   	$("#addSiteModal").modal();
			   	$("#edit_stationName").attr("readOnly",false); 
			   	$("#changeHide").hide();
			...
			//确认操作后页面刷新
			 ...
			 /* $.fd.ajax({  
	                ...
	                success: function(data) {  
	                	$("#up_station_level").append("<option></option>");
	   	 ...
	   	 function sub() {  
			 var array = new Array();
				...
				$('input[name="relationPlace"]').val("[" + array + "]");
		...
		function change() {
			...
			$("#addSiteModal").modal();
			$("#edit_stationName").attr("readOnly",true); 
			$.fd.ajax({
						type : "GET",
						...
						success : function(data) {
							...
							$("#edit_stationName").val(data.data.stationName);
							 ...
							 if(data.data.relationPlace == undefined || data.data.relationPlace == null || 
									 data.data.relationPlace == ""){
								 
							 }else{
								 var relationBeginAddress = JSON.parse(data.data.relationPlace);
								 var  i = 10000;
									...
									<tbody id="siteTbody">
										<c:forEach items="${stationList.rows}" var="it">
											<tr>
												...
												<td>
													<%-- <c:if test="${it.parentId != null}">
															<c:forEach items="${stationList.rows}" var="its">
																<c:if test="${its.id == it.parentId}">
																	${its.stationName}
																</c:if>
															</c:forEach>
													</c:if> --%>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="addForm">
						...
						<input type="hidden" name="relationPlace" />
						<div class="form-inline ">
							<div class="form-group">
								<label>&emsp;关联发运地：</label>
								<ul id="relationAddressUl">
								</ul>
							</div>
						</div>
						<hr/>
						<div class="project_info" id="accountHide">
...
<script>
	function htmlTable(results){
    		...
    		$.each(stationList,function(index,it){
    			...
    			$('#siteTbody').append("<tr><td style='text-align: center;'><label class='demo--label'>"
        			+"<input class='demo--checkbox' type='checkbox'  value='"+it.id+"'>"
                     +"<span class='demo--checkboxInput'></span>"            
                     +"</label></td>"
