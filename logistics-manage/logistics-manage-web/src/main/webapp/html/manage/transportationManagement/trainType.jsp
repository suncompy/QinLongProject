...
<script type="text/javascript">
	 ...
	 $(function(){
		//确认操作后页面刷新
			...
			$("#sure_msgs").click(function() {
				$("#promptIdModals").modal("hide");
				...
				<div class="tab-pane active" id="panel1">
					<div class="domain">
						...
						<c:forEach items='${permissions }' var='permission' >
							<c:if test="${permission.code=='addTrainType'}">
							<a href="javascript:void(0)" class="exportBtn addNew"  id="addType"><span>添加车型</span></a>
							</c:if>
							<c:if test="${permission.code=='delTrainType'}">
	...
	<div class="modal fade addTypeModal" id="addTypeModal" tabindex="-1" role="dialog" data-backdrop="static">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<h5>车型信息<i class='requireds'>*</i></h5>
	            <form id="addTypeForm">
					<div class="project_info">
						...
						<div class="form-inline">
							<div class="form-group">
								...
								<input type="text" maxlength="30" name="selfWeight" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
								<span>吨</span>
							</div>
							<div class="form-group">
								<label>&emsp;&emsp;&emsp;载重：</label>
								<input type="text" maxlength="30" name="weight" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
								<span>吨</span>
							</div>
							<div class="form-group">
								<label>&emsp;&emsp;&emsp;容积：</label>
								<input type="text" maxlength="30" name="volume" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
								<span>m³</span>
							</div>
							<div class="form-group">
								<label>预计费单位：</label>
								<input type="text" maxlength="30" name="loadPrice" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
								<span>吨</span>
							</div>
						</div>
						<div class="form-inline">
							<div class="form-group">
								...
								<input type="text" maxlength="30" name="length" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
								<span>mm</span>
							</div>
							<div class="form-group">
								<label>&emsp;&emsp;&emsp;&emsp;宽：</label>
								<input type="text" maxlength="30" name="width" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
								<span>mm</span>
							</div>
							<div class="form-group">
								<label>&emsp;&emsp;&emsp;&emsp;高：</label>
								<input type="text" maxlength="30" name="hight" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
								<span>mm</span>
							</div>
						</div>
						<div class="form-inline ">
        ...
        <!--确认提示模态框-->		
		  <div class="modal fade promptModal" tabindex="-1" role="dialog"
			...
			<div class="modal-dialog " role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title" id="myModalLabels">提示</h4>
						<span data-dismiss="modal" aria-label="Close"></span>
					</div>
					<div class="modal-body">
						<p id="msgs" style="text-align: center;font-size: 24px;"></p>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn sureBtn" id="sure_msgs">
							确定</button>
					</div>
				</div>
			</div>
		...
		callback: function(num) {
	        	...
	        	success:function(data){
	        		...
	        		$.each(tyList,function(index,it){
	        			$('#tyBody').append("<tr><td style='text-align: center;'><label class='demo--label'>"
						...
						+"<td>"+ it.selfWeight+"吨</td> <td>"+it.weight+"吨</td> <td>"+it.volume+"m³</td> <td>"+it.loadPrice+"吨</td><td>"+it.length+"mm</td> <td>"+ it.width+"mm</td> <td>"+it.hight +"mm</td> <td>"+it.remark+"</td> </tr>")
	        		})	
	        	}  
	    	}
		})
