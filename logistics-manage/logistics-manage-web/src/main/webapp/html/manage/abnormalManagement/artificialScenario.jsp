	...
	<head>
		...
		$(function(){
			$("#addScenarios").click(function(){
				...
				$("#myModalLabel").html("添加情景");
				$("#sceneName").attr("readOnly",false);
				$("#formula").hide();
				$("#relationUl").append("<li><select  name='brachId'><option  value=''></option>"+
						"<c:forEach items='${projectList }' var='branchGroup' >"+
							"<option  value='${branchGroup.id}'>${branchGroup.projectCode}"+
			...
			$("body").on("click",".addImg",function(){
				$("#relationUl").append("<li><select  name='brachId'><option  value=''></option><c:forEach items='${projectList }' var='branchGroup'><option  value='${branchGroup.id}'>${branchGroup.projectCode}</option></c:forEach></select>\
		...
		function addAbnormal(){
			...
			$('#relationUl select option:selected').each(function(j,item){
				branchArray[j] =$(item).text();
			});	
			var abnormalJsonArray = JSON.stringify(branchArray);
		 	$("#projectCode").val(abnormalJsonArray);
					...
					if(data.status==200){
						...
						$("#relationUl").append("<li><select id='branchId' name='brachId'>"+
								"<c:forEach items='${projectList }' var='branchGroup' >"+
									"<option  value='${branchGroup.id}'>${branchGroup.projectCode}"+
								"</option></c:forEach></select>");
						$("#branchId").val(data.data.projectId);
	        		...
	        		if(data.status==200){
	        			$("#page").paging({
	        				...
	        				callback: function(num) {
	        			        	...
	        			        	success:function(data){
	        			        		if(data.status==200){
	        			        		var abnormalList = data.data.rows;
	        			        		$('#abnormalBody').html("");
	        			        		$.each(abnormalList,function(index,abnormal){
	        			        			$('#abnormalBody').append("<tr>"
	        								 ...
	        								 +"<td id=''>"+abnormal.branchGroupId+"</td>"
			...
			<form class="form-inline maxWidth" id="searchForm">
				<div id="wrap">
					<div class="form-group">
						<label>关联项目：</label>
						<input name='branchGroupId' type="text" maxlength="30"/>
					</div>
				</div>
				<div class="foot">
			...
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<h5>情景信息<i class='requireds'>*</i></h5>
						...
						<input id="abnormalId" type="hidden" name="id" />
							...
							<div class="relationClass">
								...
								<input id="projectCode" type="hidden" name="projectCode" />
							</div>
		...
		<div class="modal fade promptModal" tabindex="-1" role="dialog" id="errorPromptIdModal">
			<div class="modal-dialog " role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body" style="text-align: center;color: #000000;font-size: 20px;">
			      	<i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
