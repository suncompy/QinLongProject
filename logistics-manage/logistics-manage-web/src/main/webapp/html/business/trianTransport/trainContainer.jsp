...
<html>
	<head>
		...
		<link rel="stylesheet" href="${pageContext.request.contextPath}/css/business/fireManagement/trainContainer.css" />
		...
		<script type="text/javascript" src="${pageContext.request.contextPath}/js/business/fireManagement/trainContainer.js"></script>
		...
		document.onkeydown = function () {
		  ...
		  function entruckWeightLength(obj){
			  //正则 只能输入两位整数 或者 两位整数后保留一位小数点 
			    ...
			    if(obj.value.indexOf(".")> 2 && obj.value !=""){ 
			        obj.value= parseFloat(""); 
				}  
			    //假如先填写的载重  然后再选择集装箱号，存在忘记选择情况，这时候装车信息校验会有问题 在此做了限制  
			    /* var c1 = $(obj).parent().prev().find("select").val();
			    if(c1 == "" || c1 == null || c1 == undefined){
			    	$("#errorMsg").html("请选择集装箱号");
					$("#promptIdModal").modal();
					obj.value= "";
					return;
			    } */
			   ...
			   /*  var array = new Array();
			    ...
			    var stationId = $("#hiddenBeginSiteId").val();
			    var cargoId = $(obj).parent().parent().parent().parent().prev().children().find("li").eq(0).find("select").val();
			    var siteId = $(obj).parent().parent().parent().parent().prev().children().find("li").eq(1).find("select").val();
			   ...
			   /* $.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/trainOrder/checkEntruckWeight.do",
					data : {array:array},
					dataType : 'json',
					traditional: true,
					async : false,
		  ...
		  function sumWeight2(param){
			...
			function addTrainOrder(){
				...
				$.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/trainOrder/addTrainOrder.do",
					dataType : 'json',
			...
			function delsTrainOrder(){
				...
				$.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/trainOrder/deleteTrainOrderByParam.do",
					dataType : 'json',
					data : {
						"id" : id,
						"reason":reason
			...
			function exchangeProject(){
				...
				$.fd.ajax({
					type : "GET",
					url : "${pageContext.request.contextPath}/trainOrder/selectProject.do?id="+id,
					dataType : 'json',
					success : function(data) {
			...
			$(function(){
				...
				//项目下拉
				 $.fd.ajax({  
		                type: "GET",  
		                url:"${pageContext.request.contextPath}/trainOrder/getAllProject.do?projectType="+0,  
		                success: function(data) {  
				...
				$.fd.ajax({  
		                type: "POST",  
		                url:"${pageContext.request.contextPath}/trainOrder/pleaseTrainType.do",  
		                success: function(data) {  
				...
				$("body").on("click", ".needLookDetail tr td:not(:nth-child(1)):not(:nth-child(5))", function() {
						...
						$.fd.ajax({
							type : 'GET',
							url : "${pageContext.request.contextPath}/trainOrder/trainOrderDetailById.do?id="+id+"&type="+0,
							success : function(data) {
								var sta = "";
								...
								$("#detailsUlThingInfo").append(
		   	                			...
		   	                			+"<li>&emsp;&emsp;库存：<span>600</span></li>" */
								...
								$.each(details,function(index,details){
									var sta1 = "";
									var sta2 = "";
									if(details.containerNumber2 == ""){
										sta2 = "-";
									}else{
										sta2 = details.containerNumber2;
									}
									if(details.containerNumber1 == ""){
										sta1 = "-";
									}else{
										sta1 = details.containerNumber1;
									}
									$("#detailBody").append(
											"<tr><td>"+ (index+1) +"</td><td>"+details.carType+"</td><td>"+details.carNumber+"</td><td>"+sta1+"</td><td>"+details.sendWeight+"</td><td>"+details.unloadWeight+"</td><td>"+sta2+"</td><td>"+details.conSendWeight2+"</td>"
											+"<td>"+details.conUnloadWeight2+"</td><td>"+details.arriveCargoPlaceName+"</td><td>"+details.arriveCargoSiteName+"</td><td>"+details.arriveUnloadTime+"</td></tr>");
		   	                	});
				...
				$(".abnorm").click(function() {
					...
					if($("#" + id + "  input[type=checkbox]:checked").length > 1) {
						...
						$.fd.ajax({  
		   	                type: "GET",  
		   	                url:"${pageContext.request.contextPath}/trainOrder/getCarrierMsg.do?id="+id,  
		   	                success: function(data) {
			...
			function handleException(){
				...
				$.ajax({  
   	                type: "POST",  
   	                url:"${pageContext.request.contextPath}/trainOrder/handleExceptionById.do",
			...
			function addWaitAdmit(){
				...
				$.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/trainOrder/addCarrier.do",
			...
			function loadingEntruck(id){
				...
				$.fd.ajax({  
   	                ...
   	                success: function(data) {  
   	                	...
   	                	$("#hiddenBeginSiteId").val(data.data.beginSiteId);
			...
			$(function(){
				...
				$("body").on("change",".freightYard",function(){
					...
					$.each(arrtbody, function(i,val){      
					    ...
					    for (var j = 0; j < arrTr.length; j++) {
					    	...
					    	arrTd.eq(7).html(checkValue);
						    arrTd.eq(8).html(checkText);
					    };
					});
					//等待装车 货位下拉
					$.fd.ajax({  
				...
				$("body").on("change",".freightAllocation",function(){
					...
					$.each(arrtbody, function(i,val){      
					    ...
					    for (var j = 0; j < arrTr.length; j++) {
					    	var arrTd = arrTr.eq(j).find('td');
					    	arrTd.eq(9).html(checkValue);
						    arrTd.eq(10).html(checkText);
					    }
					});
				});
			});
			...
			function gainSelectValue(param){
				...
				$(param).parent().next().next().next().next().next().next().html(fyId);
				$(param).parent().next().next().next().next().next().next().next().html(fyName);
				...
				$(param).parent().next().next().next().next().next().next().next().next().html(faId);
				$(param).parent().next().next().next().next().next().next().next().next().next().html(faName);
			}
			...
			function saveWaitTrainInput() {
				...
				/* $.each(arrtbody, function(i,val){      
				    ...
				    $.each(arrTr, function(j,trVal){    
					    ...
					    $.each(arrTd, function(j,tdVal){        
						    trainInfo += tdVal.innerHTML + '&';
						});
					    trainInfo += ']';
					});
				    allocation = allocation + '{' + trainInfo + '}';
				});  */
				var entruckWeight = 0;
				var carNum = 0;
				var containerNums =0;
				...
				var jzxArr = $(".newTr [name=jzx2]");
				for(var i=0;i<jzxArr.length;i++){
					if(jzxArr.eq(i).val()){
						containerNums = containerNums+1;
						//jzxArr.eq(i).text(jzxArr.eq(i).find("option:selected").text());
					}else{
				...
				$.each(arrtbody, function(i,val){      
				    ...
				    $.each(arrTr, function(j,trVal){    
						...
						//var container_number1 = arrTd.eq(3).text();
						var container_number1 = arrTd.eq(3).find("option:selected").text();
						var send_weight = arrTd.eq(4).find("input").val()?parseFloat(arrTd.eq(4).find("input").val()):0;
						var send_weights = arrTd.eq(4).find("input").val();
						var container_number2 = arrTd.eq(5).find("option:selected").text();
						var con_send_weight2 = arrTd.eq(6).find("input").val()?parseFloat(arrTd.eq(6).find("input").val()):0;
						var con_send_weight2s = arrTd.eq(6).find("input").val();
						var cargo_place_id = arrTd.eq(7).text();
						var cargo_place_name = arrTd.eq(8).text();
						var cargo_site_id = arrTd.eq(9).text();
						var cargo_site_name = arrTd.eq(10).text();
						saveWait.push({
							carType : car_type,
							carNumber : car_number,
							containerNumber1 : container_number1,
							sendWeight :send_weights,
							containerNumber2 : container_number2,
							conSendWeight2 :con_send_weight2s,
						...
						entruckWeight = entruckWeight + send_weight + con_send_weight2;
						carNum = carNum + cari;
				    });
				});
				$("#entruckWeight").val(entruckWeight);
				$("#entruckNumbe").val(carNum);
				$("#containerNums").val(containerNums);
				//检验表格信息是否全都填写
				 ...
				 $.each(arrtbody, function(i,val){      
				    ...
				    $.each(arrTr, function(j,trVal){    
						...
						var container_number1 = arrTd.eq(3).find("option:selected").text();
						var container_number2 = arrTd.eq(5).find("option:selected").text();
						var send_weight = arrTd.eq(4).find("input").val();
						var con_send_weight2 = arrTd.eq(6).find("input").val();
						if((car_type != "" || car_number != "" || send_weight != "" || con_send_weight2 != "") && (container_number1 == "" && container_number2 == "")){
							nullFlag = true;
						} 
						if(container_number1 != "" && container_number2 == ""){
							if((car_type == "" || car_number == "" || send_weight == "")){
								//填写了一个  集装至少一个不能为空
								allNullFlag = true;
							}
						}else if(container_number1 == "" && container_number2 != ""){
							if((car_type == "" || car_number == "" || con_send_weight2 == "")){
								//填写了一个  集装至少一个不能为空
								allNullFlag = true;
							}
						}else if(container_number1 != "" && container_number2 != ""){
							if((car_type == "" || car_number == "" || send_weight == "" || con_send_weight2 == "")){
								//两个集装箱都填写 
								allNullFlag = true;
							}
						}
						if(send_weight != "" && con_send_weight2 == ""){
							if((car_type == "" || car_number == "" || container_number1 == "")){
								//填写了一个  集装箱 装车载重 
								allNullFlag = true;
							}
						}else if(send_weight == "" && con_send_weight2 != ""){
							if((car_type == "" || car_number == "" || container_number2 == "")){
								//填写了一个  集装至少一个不能为空
								allNullFlag = true;
							}
						}else if(send_weight != "" && con_send_weight2 != ""){
							if((car_type == "" || car_number == "" || container_number1 == "" || container_number2 == "")){
								//两个集装箱都填写 
								allNullFlag = true;
							}
						}
				...
				if(allNullFlag){
					...
					$.ajax({
						type : 'POST',
						url : "${pageContext.request.contextPath}/trainOrder/addWaitEntruck.do",
			...
			function waitUnloading(id){
				...
				$.fd.ajax({  
   	                type: "GET",  
   	                url:"${pageContext.request.contextPath}/trainOrder/getSendMsg.do?id="+id,  
   	                success: function(data) {  
   	                	$("#waitUlUnloadContent").append("<input type='hidden' value='"+data.data.trainOrder.id+"' id='orderIdUnload' name='orderIdUnload'>"
   	                			...
   	                			+"<li>项目编号：<span>"+data.data.trainOrder.projectCode+"</span></li>"
   	 							...
   	 							+"<li>集装箱数：</span>"+data.data.trainOrder.containerNums+"</li>"
   	                	...
   	                	var trainCargoList = data.data["trainCargoList"];
                            ...
                            +"</label></td><td style='display:none'></td><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td><td>"+tcList.sendWeight+"</td><td><input type='text'  oninput='loadingValue1(this)' style='height:28px;border:1px solid #dde3ef; border-radius: 4px;' value="+tcList.unloadWeight+"></td>"
	   	 					+"<td>"+tcList.containerNumber2+"</td><td>"+tcList.conSendWeight2+"</td><td><input type='text'  oninput='loadingValue2(this)' style='height:28px;border:1px solid #dde3ef; border-radius: 4px;' value="+tcList.conUnloadWeight2+"></td><td><select name='freight_guide"+index+"' class='select_freight_guide' onchange='cargoLoExchangetd(this)'></select></td><td><select name='yard_guide"+index+"' onchange='yardLoExchangetd(this)'></select></td><td name='resaonSend"+index+"'></td><td name='resaonArrive"+index+"'></td></tr>")
			...
			function cargoLoExchange(){
				...
				$.each($('.unloadingChecked input:checkbox'), function() {
					if (this.checked) {
						var td_freight_guide = $(this).parent().parent().next().next().next().next().next().next().next().next().next().children();
			...
			function yardExchange(){
				 ...
				 $.each($('.unloadingChecked input:checkbox'), function() {
						if (this.checked) {
							var td_cargo_guide = $(this).parent().parent().next().next().next().next().next().next().next().next().next().next().children();
			...
			function cargoLoExchangetd(param){
				...
				//改变td中卸货货场 给隐藏td赋值id
				if(id == "" || id == null || id == undefined){
					$(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().html("");
				}else{
					var unloadIdc = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
					var unloadHideIdc = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev();
			...
			function yardLoExchangetd(param){
				var unloadIdy = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideIdy = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev();
				unloadHideIdy.html(unloadIdy);
			}
			
			//输入到货载重给隐藏td赋值id
			function loadingValue1(param){
				var sendWeight = $(param).parent().prev().text();
				var c1Num = $(param).parent().prev().prev().text();
				var unloadId = $(param).parent().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideId = $(param).parent().prev().prev().prev().prev().prev().prev();
				if(param.value == null || param.value ==""){
					unloadHideId.html("");
				}else{
					unloadHideId.html(unloadId);
					if(c1Num == "" || c1Num == null || c1Num == undefined){
						$("#errorMsg").html("未有集装箱号，不可填入到货载重");
						$("#promptIdModal").modal();
						param.value="";
						unloadHideId.html("");
						return;
			...
			//输入到货载重给隐藏td赋值id
			function loadingValue2(param){
				var sendWeight = $(param).parent().prev().text();
				var c2Num = $(param).parent().prev().prev().text();
				var unloadId = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideId = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev();
				if(param.value == null || param.value ==""){
					unloadHideId.html("");
				}else{
					unloadHideId.html(unloadId);
					if(c2Num == "" || c2Num == null || c2Num == undefined){
						$("#errorMsg").html("未有集装箱号，不可填入到货载重");
						$("#promptIdModal").modal();
						param.value="";
						unloadHideId.html("");
			...
			function allocatCargoAndYard(status) {
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if(tdArr.eq(1).html() == null || tdArr.eq(1).html() == ""){
						...
						var container_number1 = tdArr.eq(5).html();
						var container_number2 = tdArr.eq(8).html();
						var unload_weight = tdArr.eq(7).find("input").val();
						var con_unload_weight2 = tdArr.eq(10).find("input").val();
						var arrive_cargo_place_id = tdArr.eq(11).find("select").val();
						var arrive_cargo_place_name = tdArr.eq(11).find("option:selected").text();
						var arrive_cargo_site_id = tdArr.eq(12).find("select").val();
						var arrive_cargo_site_name = tdArr.eq(12).find("option:selected").text();
						var send_img = tdArr.eq(13).find("input").val();
						var unload_img = tdArr.eq(14).find("input").val();
						ist.push({
							id : id,
							...
							conUnloadWeight2 :con_unload_weight2,
				...
				//勾选再次确认，检验表格信息是否全都填写
				 ...
				 for(var i = 0; i < trList.length; i++){
					...
					var container_number1 = tdArr2.eq(5).html();
					var container_number2 = tdArr2.eq(8).html();
					var unload_weight = tdArr2.eq(7).find("input").val();
					var con_unload_weight2 = tdArr2.eq(10).find("input").val();
					var arrive_cargo_place_name2 = tdArr2.eq(11).find("option:selected").text();
					var arrive_cargo_site_name2 = tdArr2.eq(12).find("option:selected").text();
					//有可能集装箱只有一个的情况下
					if(container_number1 == "" && container_number2 != ""){
						if(con_unload_weight2 == "" || arrive_cargo_place_name2 == "" || arrive_cargo_site_name2 == ""){
							nullFlag = true;
							break;
						}
					}else if(container_number1 != "" && container_number2 == ""){
						if(unload_weight == "" || arrive_cargo_place_name2 == "" || arrive_cargo_site_name2 == ""){
							nullFlag = true;
							break;
						}
					}else{
						//两个集装箱 
						if(unload_weight == "" || con_unload_weight2 == "" || arrive_cargo_place_name2 == "" || arrive_cargo_site_name2 == ""){
							nullFlag = true;
							break;
						}
					}
				...
				if(flag){
					/* for(var j = 0;j < checkIst.length; j++){
						if(checkIst[j] == "" || checkIst[j] == null){
							nullFlag = true;
							break;
						}
					} */
					if(nullFlag){
						...
						if(nullFlagTwo){
							$.ajax({
								type : 'POST',
								url : "${pageContext.request.contextPath}/trainOrder/updOrderStatusByParam.do",
							...
							$.ajax({
								type : 'POST',
								url : "${pageContext.request.contextPath}/trainOrder/updateUnloadWeight.do",
					...
					$.ajax({
						...
						success : function(data) {
							if (data.status != 200) {
								...
								$.fd.ajax({  
				   	                type: "GET",  
				   	                url:"${pageContext.request.contextPath}/trainOrder/getSendMsg.do?id="+train_order_id,  
				   	                success: function(data) {  
				   	                	var trainCargoList = data.data["trainCargoList"];
					   	 				$.each(trainCargoList,function(index,tcList){
					   	 					$("#unloadingBody").append("<tr><td style='text-align: center;'><label class='demo--label'>"
				                            ...
				                            +"</label></td><td style='display:none'></td><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td><td>"+tcList.sendWeight+"</td><td><input type='text'  oninput='loadingValue1(this)'  style='height:28px;border:1px solid #dde3ef; border-radius: 4px;' value="+tcList.unloadWeight+"></td>"
					   	 					...
					   	 					$("td[name='resaonSend"+index+"']").html("");
					   	 					if(tcList.sendImg == null || tcList.sendImg ==""){
					   	         				$("td[name='resaonSend"+index+"']").append("<a href='javascript:void(0)' id='unloadSerial"+(index+1)+"' class='uploadImg' onclick='uploadUnloadImg("+(index+1)+")'>点击上传</a><input type='hidden' id='hiddenUnoladId"+(index+1)+"' value=''/>");
					   	 					}else{
					   	 						$("td[name='resaonSend"+index+"']").append("<a href='javascript:void(0)' class='lookClass' name='"+tcList.sendImg+"' onclick='lookingImg(this)'>查看</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp<a href='javascript:void(0)' class='delA' onclick='deleteSendImg("+tcList.id+","+tcList.trainOrderId+")'>删除</a>"); 
					   	         			}
						   	 				$("td[name='resaonArrive"+index+"']").html("");
					   	 					if(tcList.unloadImg == null || tcList.unloadImg ==""){
					   	         				$("td[name='resaonArrive"+index+"']").append("<a href='javascript:void(0)' id='unloadSerialTwo"+(index+1)+"' class='uploadImg' onclick='arriveUnloadImg("+(index+1)+")'>点击上传</a><input type='hidden' id='hiddenUnoladIdTwo"+(index+1)+"' value=''/>");
					   	 					}else{
					   	 						$("td[name='resaonArrive"+index+"']").append("<a href='javascript:void(0)' class='lookClass' name='"+tcList.unloadImg+"' onclick='lookingImg(this)'>查看</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp<a href='javascript:void(0)' class='delA' onclick='deleteArriveImg("+tcList.id+","+tcList.trainOrderId+")'>删除</a>"); 
					   	         			}
					   	 					//td中的货场货位联动
			...
			function receipting(id){
				...
				$.fd.ajax({  
   	                ...
   	                success: function(data) {  
   	                	...
   	                	var trainCargoList = data.data["trainCargoList"];
	   	 				$.each(trainCargoList,function(index,tcList){
	   	 					$("#receiptionBody").append("<tr><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td><td>"+tcList.sendWeight+"</td><td>"+tcList.unloadWeight+"</td>"
	   	 					+"<td>"+tcList.containerNumber2+"</td><td>"+tcList.conSendWeight2+"</td><td>"+tcList.conUnloadWeight2+"</td><td name='resaonSends"+index+"'></td><td name='resaonArrives"+index+"'></td></tr>")
			...
			function resetTrainOrder(){
				...
				$.ajax({
					type : 'POST',
					url : "${pageContext.request.contextPath}/trainOrder/resetDeleteOrder.do",
					data : {"id":id},
					dataType : "json",
					success : function(data) {
						if(data.status==200){
			...
			function sending(id){
				...
				$.fd.ajax({  
   	                ...
   	                success: function(data) { 
   	                	...
   	                	var trainCargoList = data.data["trainCargoList"];
						$.each(trainCargoList,function(index,tcList){
							$("#trainCargoBody").append("<tr><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td>"
							+"<td>"+tcList.sendWeight+"</td><td>"+tcList.containerNumber2+"</td><td>"+tcList.conSendWeight2+"</td><td name='resaons"+index+"'></td></tr>")
			...
			function carrying(id){
				...
				$.fd.ajax({  
   	                type: "GET",  
   	                url:"${pageContext.request.contextPath}/trainOrder/getHistoryLocationMsg.do?id="+id+"&type="+0,  
   	                success: function(data) { 
   	                	var historyLocation = data.data["historyLocation"];
   	                	$.each(historyLocation,function(index,hlocationList){
						...
						$.each(hlTrainCargoList,function(index,tcList){
							$("#localtionBody").append("<tr><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td>"
							+"<td>"+tcList.sendWeight+"</td><td>"+tcList.containerNumber2+"</td><td>"+tcList.conSendWeight2+"</td><td name='reason"+index+"'></td></tr>")
			...
			function updHistoryLocation(){
				...
				$.ajax({
					type : 'POST',
					url : "${pageContext.request.contextPath}/trainOrder/updateLocation.do",
					...
					success : function(data) {
						if(data.status==200){
							...
							$.fd.ajax({  
			   	                type: "GET",  
			   	                url:"${pageContext.request.contextPath}/trainOrder/getHistoryLocationMsg.do?id="+orderIdLocation+"&type="+0,  
			   	                success: function(data) { 
			   	                	$("#localtionUlContent").empty();
			   	                	var historyLocation = data.data["historyLocation"];
			   	                	$.each(historyLocation,function(index,hlocationList){
				...
				$.ajax({
					...
					success : function(data) {
						if(data.status==200){
							...
							$.fd.ajax({  
			   	                type: "GET",  
			   	                url:"${pageContext.request.contextPath}/trainOrder/getHistoryLocationMsg.do?id="+orderIdLocation2+"&type="+0,  
			   	                success: function(data) { 
			   	                	$("#localtionUlContent2").empty();
			   	                	var historyLocation = data.data["historyLocation"];
			   	                	$.each(historyLocation,function(index,hlocationList){
			...
			function deleteWaitSendImg(id,trainId){
				$.ajax({
					type : 'POST',
					url : "${pageContext.request.contextPath}/trainOrder/deleteSendImg.do",
					data : {"id":id},
					dataType : "json",
					success : function(data) {
						if(data.status==200){
							...
							$.fd.ajax({  
			   	                type: "GET",  
			   	                url:"${pageContext.request.contextPath}/trainOrder/getSendMsg.do?id="+trainId,  
			   	                success: function(data) { 
			   	                	var trainCargoList = data.data["trainCargoList"];
									$.each(trainCargoList,function(index,tcList){
										$("#trainCargoBody").append("<tr><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td>"
				...
				$.ajax({
					...
					success : function(data) {
						if(data.status==200){
							...
							$.fd.ajax({  
			   	                type: "GET",  
			   	                url:"${pageContext.request.contextPath}/trainOrder/getHistoryLocationMsg.do?id="+trainId+"&type="+0,  
			   	                success: function(data) { 
			   	                	var hlTrainCargoList = data.data["hlTrainCargoList"];
									$.each(hlTrainCargoList,function(index,tcList){
										$("#localtionBody").append("<tr><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td>"
			...
			function deleteReceiveArriveImg(id,trainId){
				$.ajax({
					type : 'POST',
					url : "${pageContext.request.contextPath}/trainOrder/deleteArriveImg.do",
					data : {"id":id},
					dataType : "json",
					success : function(data) {
						if(data.status==200){
			...
			function uploadUnloadImg(par,param){
				...
				/* if($("#unloadSerial"+param+"").html() == "重新上传"){
				...
				$("#hiddenTdId").val("");
				$("#hiddenTdId").val(par);
				var unloadIdsi = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideIdsi = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev();
			...
			function arriveUnloadImg(par,param){
				...
				/* if($("#unloadSerialTwo"+param+"").html() == "重新上传"){
				...
				var unloadIdai = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideIdai = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev();
			...
			function updCarringImg(){
				...
				if(sendImgId != null || sendImgId != ""){
					...
					$.ajax({
						type : 'POST',
						url : "${pageContext.request.contextPath}/trainOrder/updateSendImg.do",
						...
						success : function(data) {
							if(data.status==200){
								...
								$.fd.ajax({  
				   	                type: "GET",  
				   	                url:"${pageContext.request.contextPath}/trainOrder/getSendMsg.do?id="+p,  
				   	                success: function(data) { 
			...
			function updSendImg(){
				...
				if((sendImgId != null || sendImgId != "") && (arriveImgId == null || arriveImgId == "")){
					...
					$.ajax({
						type : 'POST',
						url : "${pageContext.request.contextPath}/trainOrder/updateArriveImg.do",
	...
	<script>
		...
		function search(param){
			...
			if(orderStatus == 1){
				...
				$.ajax({  
		        	type:'post',      
		       		url:'${pageContext.request.contextPath}/trainOrder/selectTrainOrderByPage.do',  
				...
				$.ajax({  
		        	type:'post',      
		       		url:'${pageContext.request.contextPath}/trainOrder/selectExceptionOrderByPage.do',  
			        data:{page:nums,trainOrderSearch:JSON.stringify($("#searchForm").serializeJSON())},
			        cache:false,
			        dataType:'json',
		        	success:function(data){
				...
				$.ajax({  
		        	type:'post',      
		       		url:'${pageContext.request.contextPath}/trainOrder/selectHistoryOrderByPage.do',  
			        data:{page:nums,trainOrderSearch:JSON.stringify($("#searchForm").serializeJSON())},
			        cache:false,
			        dataType:'json',
		        	success:function(data){
				...
				$.ajax({  
		        	type:'post',      
		       		url:'${pageContext.request.contextPath}/trainOrder/selectDeleteOrderByPage.do',  
			        data:{page:nums,trainOrderSearch:JSON.stringify($("#searchForm").serializeJSON())},
			        cache:false,
			        dataType:'json',
		        	success:function(data){
				...
				<div class="tab-content">
					<div class="tab-pane active" id="panel1">
						<div class="domain">
							...
							<c:forEach items='${permissions }' var='permission' >
								<c:if test="${permission.code=='createTrainContainerOrder'}">
									<a href="javascript:void(0)" class="exportBtn add" id="add" ><span>新建</span></a>
								</c:if>
								<c:if test="${permission.code=='exportTrainContainerOrder'}">
									<a href="javascript:void(0)" class="export" id="export1"><span>导出</span></a>
								</c:if>
								<c:if test="${permission.code=='delTrainContainerOrder'}">
									<a href="javascript:void(0)" class="exportBtn del" id="delBtn"><span>删除</span></a>
								</c:if>
								<c:if test="${permission.code=='msgByTrainContainerOrder'}">
					...
					<div class="tab-pane" id="panel2">
						<div class="domain">
							<div class="tableBg">
							<c:forEach items='${permissions }' var='permission' >
								<c:if test="${permission.code=='exceptionTrainContainerOrder'}">
									<a href="javascript:void(0)" class="exportBtn abnormal_btn abnorm"><span>异常处理</span></a>
								</c:if>
								<c:if test="${permission.code=='exportExceptionTrainContainerOrder'}">
									<a href="javascript:void(0)" class="export" id="export2"><span>导出</span></a>
								</c:if>
								<c:if test="${permission.code=='msgByExceptionTrainContainerOrder'}">
							...
							<div class="table-responsive">
								<!--异常运单列表表格开始-->
								<table class="table" id="exceptionTable">
									<thead>
										<tr class="tableTop">
											<th>
												<label class="demo--label">
					...
					<div class="tab-pane" id="panel3">
						<div class="domain">
							<div class="tableBg">
							<c:forEach items='${permissions }' var='permission' >
								<c:if test="${permission.code=='exportHistoryTrainContainerOrder'}">
									<a href="javascript:void(0)" class="export" id="export3"><span>导出</span></a>
								</c:if>
								<c:if test="${permission.code=='msgByHistoryTrainContainerOrder'}">
					...
					<div class="tab-pane" id="panel4">
						<div class="domain">
							<div class="tableBg">
							<c:forEach items='${permissions }' var='permission' >
								<c:if test="${permission.code=='exportRecycleTrainContainerOrder'}">
									<a href="javascript:void(0)" class="export" id="export4"><span>导出</span></a>
								</c:if>
								<c:if test="${permission.code=='msgByRecycleTrainContainerOrder'}">
						...
						<div class="project_info">
							...
							<div class="form-inline">
								<div class="form-group">
									...
									<select id="chooseAdvanceChargeAccount" onchange="exchangeAdvanceNum()" ></select>
								</div>
								<div class="form-group">
									<label>预付款余额：</label>
					...
					<div class="modal-body" id="waitUlLoadForm">
						...
						<div class="project_info">
							...
							<ul class="waitUl" id="waitUlLoadContent">
								...
								<li><span class="v-top">集装箱数：</span><input type="text" maxlength="30" id="containerNums" name="containerNums" onkeyup="value=value.replace(/[^\d]/g,'') " readonly="readonly"/></li>
							...
							<input type="hidden"  id="hiddenBeginSiteId"  />
						</div>
						<hr />
						...
						<input id="hiddenPath" type="hidden" value="${pageContext.request.contextPath}" />
							<div class="project_info">
								<ul class="waitUl">
									<li>装车货场：
