    			...
    			if($("#operatorTable input:checkbox[name='chekcProject']:checked").length > 1) {
    				...
    				$.ajax({
    					...
    					success : function(data) {
	        				...
	        				var unitName = "";
	        				if(data.data.tp.valuationUnitName ==0){
	        					unitName="吨";
							}else if(data.data.tp.valuationUnitName ==1){
								unitName ="位";
							}
							...
							if(data.data.tp.projectType ==0){
								...
								$.each(detailInfo,function(index,details){
									$("#detailInfoBody").append(
											...
											+"<td>"+details.cargoPlaceName+"</td><td>"+details.cargoSiteName+"</td><td>"+details.sendWeight+"</td><td>"+details.arriveUnloadTime+"</td><td>"+details.arriveCargoPlaceName+"</td>"
											+"<td>"+details.arriveCargoSiteName+"</td><td>"+details.unloadWeight+"</td><td>"+details.wastage+"</td></tr>");
		   	                	});
							}else if(data.data.tp.projectType ==1){
								$("#containerTable").hide();
...
<script>
	//分页
		...
		callback: function(num) {
		        	...
		        	success:function(data){
		        		if(data.status==200){
		        		var accessStorageList = data.data;
		        		...
		        		$.each(accessStorageList,function(index,list){
	        				$('#accessStorageBody').append("<tr>"
									+"<td style='text-align: center;'><label class='demo--label'>"
									 +" <input class='demo--checkbox'  name='WaybillName' type='checkbox' value='"+list.projectId+"'  >"
		                              +"<span class='demo--checkboxInput'></span>"
		                             +"</label></td>"
		...
		function search(param){
			...
			$.ajax({
	        	type:'post',      
	       		url:'${pageContext.request.contextPath}/trainAccessStorage/trainAccessStorageListByPage.do',  
		        data:{page:num,trainOrderDetail:search},
		        cache:false,
		        dataType:'json',
	        	success:function(data){
	        		var accessStorageList = data.data;
	        		...
	        		$.each(accessStorageList,function(index,list){
	        			$('#accessStorageBody').append("<tr>"
		        			...
		        			$("td[name='projectType"+index+"']").html("");
		        			if(list.projectType==0){
		        				$("td[name='projectType"+index+"']").append("<a>集装箱</a>"); 
		        			}
		        			else if(list.projectType==1){
		        				$("td[name='projectType"+index+"']").append("<a>散装箱</a>"); 
		        			}
		        			$("td[name='transportType"+index+"']").html("");
		        			if(list.transportType==3){
		        				$("td[name='transportType"+index+"']").append("<a>火运</a>"); 
		        			}
		        			else if(list.transportType==4){
		        				$("td[name='transportType"+index+"']").append("<a>接取+火运</a>"); 
		        			}
		        			else if(list.transportType==5){
		        				$("td[name='transportType"+index+"']").append("<a>火运+送达</a>"); 
		        			}
		        			else if(list.transportType==6){
		        				$("td[name='transportType"+index+"']").append("<a>联运</a>"); 
		        			}
	        		});	
	        	}
			});
