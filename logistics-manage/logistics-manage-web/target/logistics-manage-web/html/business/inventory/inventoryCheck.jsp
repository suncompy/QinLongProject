...
<head>
	...
	<script>
		function search(param){
			...
			$.ajax({
	        	type:'post',      
	       		url:'${pageContext.request.contextPath}/inventory/listProjectInventoryByPage.do',  
					...
					<div class="tab-pane active" id="panel1">
						...
						<div class="domain">
							<div class="tableBg">
								...
								<a href="javascript:;" class="exportBtn adjust" id="stockAdjust"><span>库存调整</span></a>
							...
							<div class="table-responsive">
								<table class="table">
									...
									<tbody id='projectTbody'>
										<c:forEach items="${inventoryChecks.rows}" var="stock">
										<tr>
											<td id="${stock.projectId}">
												<label class="demo--label">
								            <input class="demo--checkbox"  transportType='${stock.transportType}' value="${stock.projectId}" type="checkbox"  >
							                  <span class="demo--checkboxInput"></span>
							                         	</label></td>
											<td ><a href='#'> ${stock.projectCode}</a></td>
											<c:if  test="${stock.projectType==0}">
													<td>集装箱</td>
											</c:if>
											<c:if  test="${stock.projectType==1}">
													<td>散装</td>
											</c:if>
											<c:if  test="${stock.transportType==0}">
														<td>汽运</td>
											</c:if>
											<c:if  test="${stock.transportType==1}">
													<td>接取</td>
											</c:if>
											<c:if  test="${stock.transportType==2}">
													<td>送达</td>
											</c:if>
											<c:if  test="${stock.transportType==3}">
													<td>火运</td>
											</c:if>
											<c:if  test="${stock.transportType==4}">
													<td>接取+火运</td>
											</c:if>
											<c:if  test="${stock.transportType==5}">
													<td>火运+送达</td>
											</c:if>
											<c:if  test="${stock.transportType==6}">
													<td>联运</td>
											</c:if>
											<c:if  test="${stock.transportType==7}">
													<td>接取+送达</td>
											</c:if>
											<td id="">${stock.branchGroupName}</td>
											<td id="">${stock.cargoName}</td>
											<td id="">${stock.sendCargoCompanyName}</td>
											<td id="">${stock.beginStationName}</td>
											<td id="">${stock.beginFreightYardName}</td>
											<td id="">${stock.beginCargoLocationName}</td>
											<td id="">${stock.beginEnterQty}</td>
											<td id="">${stock.beginOutQty}</td>
											<td id="">${stock.beginCurrentQty}</td>
											<td id="">${stock.beginAdjustQty}</td>
											<td id="">${stock.receiveCargoCompanyName}</td>
											<td id="">${stock.endStationName}</td>
											<td id="">${stock.endFreightYardName}</td>
											<td id="">${stock.endCargoLocationName}</td>
											<td id="">${stock.endEnterQty}</td>
											<td id="">${stock.endOutQty}</td>
											<td id="">${stock.endCurrentQty}</td>
											<td id="">${stock.endAdjustQty}</td>
											<td id=""><fmt:formatDate value="${stock.adjustDate}" pattern="yyyy.MM.dd"/></td>
										</tr>
										</c:forEach>
									</tbody>
								</table>
			 ...
			 $.ajax({
					...
					success : function(data) {
						if(data.status==200){
							...
							var stationId ="";
							var stationName ="";
							if(project.transportType==3 || project.transportType==5){//火运 火运+送达
								stationId=project.beginSiteId;
								stationName=project.beginSiteName;
								$("#storageEnterModal input[name=address]").val(project.beginAddress);
							}else{
								stationId=project.forwardingSiteId;
								stationName=project.forwardingSiteName;
								$("#storageEnterModal input[name=address]").val(project.sendTrainStation.adressCode+project.sendTrainStation.detailAddress);
							}
							$("#storageEnterModal input[name=stationId]").val(stationId);
							$("#storageEnterModal input[name=stationName]").val(stationName);
							
							$("#storageEnterModal input[name=centerStation]").val(project.beginCenterSiteName);
	...
	<script>
		function getCargoLocation(param){
			...
			alert($(param).find('option:checked').text());
			alert(id);
			$.ajax({
				type : 'GET',
					...
					if(data.status==200){
						...
						$("#storageEnterModal select[name=cargoLocationId]").html("");
							$.each(cargoLocations, function(index,cargoLocation) {
								$("#storageEnterModal select[name=cargoLocationId]").append("<option value='"+cargoLocation.id+"'>"+cargoLocation.name+"</option>");
							});
					}else{
						$("#msgContent").html("");
		    			$("#msgContent").html(data.msg);
			 ...
			 $.ajax({
	        	...
	        	success:function(data){
	        		...
	        		if(stocks!=undefined && stocks!=null){
						...
						+"<img height='218px' width='220px' class='img-responsive' src='${pageContext.request.contextPath}/upload/photo/"+stock.img+"' alt='' />"
	...
	/* 	$("#adjustId table td:nth-child(7)").dblclick(function(){ */
			//alert(123);
			var td = $(this);
	...
	$("#page").paging({
		pageNo:1,
		totalPage:${inventoryChecks.totalPage},
		totalSize:${inventoryChecks.limit},
		callback: function(num) {
			$.ajax({
	        	type:'post',      
	       		url:'${pageContext.request.contextPath}/inventory/listProjectInventoryByPage.do', 
...
<script>
	...
	function htmlTable(results){
		$('#projectTbody').html("");
		$.each(results,function(index,stock){
		//项目类型
		var projectType="";
		if(stock.projectType==0){
			projectType="集装箱";
		...
		$('#projectTbody').append("<tr><td><label class='demo--label'>"
		+"<input class='demo--checkbox' type='checkbox' value='"+stock.projectId+"' name='WaybillName'>"
