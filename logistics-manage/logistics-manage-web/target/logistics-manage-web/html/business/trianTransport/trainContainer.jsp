...
<html>
	<head>
		...
		<script type="text/javascript">
				...
				$.ajax({
					...
					success : function(data) {
						...
						if(data.data.stock == 1){
							...
							$("#newModalBody input[name='moreHuo']").val(data.data.stock.freightYardName);
							$("#newModalBody input[name='moreWei']").val(data.data.stock.cargoLocationName);
							$("#newModalBody input[name='kuCun']").val(data.data.stock.currentQty);	
						}
					}
				});
			}
			...
			function advanceAmount(){
				...
				if(hideEstimateCost == null || hideEstimateCost == ""){
					hideEstimateCost = 0;
				}
				$("#estimateCost").val((parseInt(hideEstimateCost)*parseInt(pleaseTrainNum)));
				if(pleaseTrainNum==null || pleaseTrainNum==""){
					$("#estimateCost").val("");
				}
			}
						...
						$.ajax({
							...
							success : function(data) {
								...
								if(data.data.stocks == 1){
									...
									cargoPlaceDetail=data.data.stocks.freightYardName;
									yardDetail=data.data.stocks.cargoLocationName;
									kucunDetail=data.data.stocks.currentQty;
								} 
								...
								$("#detailsUlThingInfo").append(
		   								...
		   								+"<li>运输费用：<span>100</span></li>");
								$("#detailBody").empty();
								var details = data.data["detail"];
								$.each(details,function(index,details){
									$("#detailBody").append(
											"<tr><td>"+ (index+1) +"</td><td>"+details.carType+"</td><td>"+details.carNumber+"</td><td>"+details.containerNumber1+"</td><td>"+details.containerNumber2+"</td><td>"+details.sendWeight+"</td>"
				...
				$.ajax({  
   	                ...
   	                success: function(data) {  
   	                	$("#waitUlContent").append("<input type='hidden' value='"+data.data.trainOrder.id+"' id='orderIdWait' name='orderIdWait'>"
   								...
   								+"<input type='hidden' value='"+data.data.trainOrder.pleaseCarNum+"' id='hiddenPleaseNum'>"
   								+"<li><span class='v-top'>承认车数：</span><input type='text' id='sureNum' name='sureNum' maxlength='30'/></li>");
			...
			function loadingEntruck(id){
				...
				$.ajax({  
   	                ...
   	                url:"${pageContext.request.contextPath}/trainOrder/getCarrierMsg.do?id="+id,  
   	                success: function(data) {  
   	                	$("#waitUlLoadContent").append("<input type='hidden' value='"+data.data.trainOrder.id+"' id='orderId' name='orderId'>"
   	                			+"<li>项目编号：<span>"+data.data.trainOrder.projectCode+"</span></li>"
   								+"<li>请车单号：<span>"+data.data.trainOrder.pleaseTrainNumber+"</span></li>"
   								+"<li><span class='v-top'>&emsp;装车数：</span><input type='text' id='entruckNumbe' name='entruckNumbe' maxlength='30'/></li>"
   								+"<li><span class='v-top'>集装箱数：</span><input type='text' id='containerNums' name='containerNums' maxlength='30'/></li>"
   								+"<li><span class='v-top'>装载吨位：</span><input type='text' id='entruckWeight' name='entruckWeight' maxlength='30'/></li>"
			...
			$(function(){
					...
					$.ajax({  
			             ...
			             success: function(data) {  
			 				...
			 				$.each(data.data, function(index, its) {
			 					newFa.append("<option value="+its.id+">" + its.name + "</option>");
			 				});
			 				newFa.change();
			             }
			         }); 
				...
				$("body").on("change",".freightAllocation",function(){
					...
					$.each(arrtbody, function(i,val){      
					    ...
					    for (var j = 0; j < arrTr.length; j++) {
					    	var arrTd = arrTr.eq(j).find('td');
					    	arrTd.eq(8).html(checkValue);
						    arrTd.eq(9).html(checkText);
					    }
					});
				});
			});
			...
			function gainSelectValue(param){
				var fyId = $(param).parent().parent().parent().prev().children().find("li").eq(0).find("select").val();
				var fyName = $(param).parent().parent().parent().prev().children().find("li").eq(0).find("option:selected").text();
				$(param).next().next().next().next().next().html(fyId);
				$(param).next().next().next().next().next().next().html(fyName);
				
				var faId = $(param).parent().parent().parent().prev().children().find("li").eq(1).find("select").val();
				var faName = $(param).parent().parent().parent().prev().children().find("li").eq(1).find("option:selected").text();
				$(param).next().next().next().next().next().next().next().html(faId);
				$(param).next().next().next().next().next().next().next().next().html(faName);
			}
			...
			function saveWaitTrainInput() {
				...
				$.each(arrtbody, function(i,val){      
				    ...
				    $.each(arrTr, function(j,trVal){    
					    var arrTd = $(trVal).find('td');//每一行的td数组 赋值
					    var car_type = arrTd.eq(1).html();
					    var car_number = arrTd.eq(2).html();
						var container_number1 = arrTd.eq(3).html();
						var container_number2 = arrTd.eq(4).html();
						var send_weight = arrTd.eq(5).html();
						var cargo_place_id = arrTd.eq(6).html();
						var cargo_place_name = arrTd.eq(7).html();
						var cargo_site_id = arrTd.eq(8).html();
						var cargo_site_name = arrTd.eq(9).html();
						saveWait.push({
							carType : car_type,
				...
				$.ajax({  
   	                ...
   	                success: function(data) {  
   	                	...
   	                	var trainCargoList = data.data["trainCargoList"];
                            ...
                            +"</label></td><td style='display:none'></td><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td>"
	   	 					+"<td>"+tcList.containerNumber2+"</td><td>"+tcList.sendWeight+"</td><td contentEditable='true' onkeyup='loadingValue(this)'>"+tcList.unloadWeight+"</td><td><select name='freight_guide"+index+"' class='select_freight_guide' onchange='cargoLoExchangetd(this)'></select></td><td><select name='yard_guide"+index+"' onchange='yardLoExchangetd(this)'></select></td><td name='resaonSend"+index+"'></td><td name='resaonArrive"+index+"'></td></tr>")
				   	 				...
				   	 				$.ajax({  
				   	 		             ...
				   	 		             success: function(data) {  
				   	 		            	$("select[name='yard_guide"+index+"']").empty();
				   	 		 				...
				   	 		 				$("select[name='yard_guide"+index+"']").append("<option value="+itj.id+">" + itj.name + "</option>");
				   	 		 				});
			...
			function cargoLoExchangetd(param){
				...
				$.ajax({  
		             ...
		             success: function(data) {  
		            	 location.empty();
		 				$.each(data.data, function(index, its) {
		 					location.append("<option value="+its.id+">" + its.name + "</option>");
		 				});
		             }
		         }); 
				//改变td中卸货货场 给隐藏td赋值id
				var unloadIdc = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideIdc = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev();
			...
			function yardLoExchangetd(param){
				var unloadIdy = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideIdy = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev();
				unloadHideIdy.html(unloadIdy);
			}
			
			//输入到货载重给隐藏td赋值id
			function loadingValue(param){
				var unloadId = $(param).prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideId = $(param).prev().prev().prev().prev().prev().prev().prev();
			...
			function allocatCargoAndYard(status) {
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if(tdArr.eq(1).html() == null || tdArr.eq(1).html() == ""){
						...
						var container_number2 = tdArr.eq(6).html();
						var unload_weight = tdArr.eq(8).html();
						var arrive_cargo_place_id = tdArr.eq(9).find("select").val();
						var arrive_cargo_place_name = tdArr.eq(9).find("option:selected").text();
						var arrive_cargo_site_id = tdArr.eq(10).find("select").val();
						var arrive_cargo_site_name = tdArr.eq(10).find("option:selected").text();
						var send_img = tdArr.eq(11).find("input").val();
						var unload_img = tdArr.eq(12).find("input").val();
						ist.push({
							id : id,
				...
				//勾选再次确认，检验表格信息是否全都填写
				 ...
				 for(var i = 0; i < trList.length; i++){
					...
					var unload_weight2 = tdArr2.eq(8).html();
					var arrive_cargo_place_name2 = tdArr2.eq(9).find("option:selected").text();
					var arrive_cargo_site_name2 = tdArr2.eq(10).find("option:selected").text();
			...
			function receipting(id){
				...
				$.ajax({  
   	                ...
   	                success: function(data) {  
   	                	...
   	                	var trainCargoList = data.data["trainCargoList"];
	   	 				$.each(trainCargoList,function(index,tcList){
	   	 					$("#receiptionBody").append("<tr><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td><td>"+tcList.containerNumber1+"</td>"
	   	 					+"<td>"+tcList.containerNumber2+"</td><td>"+tcList.sendWeight+"</td><td>"+tcList.unloadWeight+"</td><td name='resaonSends"+index+"'></td><td name='resaonArrives"+index+"'></td></tr>")
				...
				$.ajax({  
   	                ...
   	                success: function(data) { 
						...
						$.each(trainCargoList,function(index,tcList){
							...
							+"<td>"+tcList.containerNumber2+"</td><td>"+tcList.sendWeight+"</td><td name='resaons"+index+"'></td></tr>")
				...
				$.ajax({  
   	                ...
   	                success: function(data) { 
						...
						$.each(hlTrainCargoList,function(index,tcList){
							...
							+"<td>"+tcList.containerNumber2+"</td><td>"+tcList.sendWeight+"</td><td name='reason"+index+"'></td></tr>")
			...
			function lookingImg(param){
	   		 	$("#lookImg").modal();
	   		 	$("#look_img").attr('src',"/img/"+param.name);
	   		}
			//等待回单上传图片模态框
			...
			function uploadUnloadImg(par,param){
				...
				$("#hiddenTdId").val("");
				$("#hiddenTdId").val(par);
				var unloadIdsi = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideIdsi = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev();
			...
			function arriveUnloadImg(par,param){
				...
				var unloadIdai = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideIdai = $(param).parent().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev().prev();
	...
	<body>
				...
				<div class="tab-content">
					<div class="tab-pane active" id="panel1">
						<div class="domain">
							<div class="tableBg">
								...
								<a href="javascript:void(0)" class="exportBtn export"><span>导出</span></a>
								<a href="javascript:void(0)" class="exportBtn del" id="delBtn"><span>删除</span></a>
								<a href="javascript:void(0)" class="exportBtn msgbtn msg"><span>短信告知</span></a>
							</div>
					...
					<div class="tab-pane" id="panel2">
						<div class="domain">
							<div class="tableBg">
								...
								<a href="javascript:void(0)" class="exportBtn export"><span>导出</span></a>
								<!-- <a href="javascript:void(0)" class="exportBtn del" id='abnormalBtn'><span>删除</span></a> -->
								<a href="javascript:void(0)" class="exportBtn msgbtn msg"><span>短信告知</span></a>
							</div>
							<div class="table-responsive">
								<!--异常运单列表表格开始-->
										...
										<c:forEach var="exception" items="${exceptionResult.rows}">
											<tr>
												...
												<td>${exception.projectCode}</td>
													...
													<td>
														<c:if test="${exception.status == 1}"><a href="javascript:void(0)">等待承认</a></c:if>
														<c:if test="${exception.status == 2}"><a href="javascript:void(0)">等待装车</a></c:if>
														<c:if test="${exception.status == 3}"><a href="javascript:void(0)">等待发运</a></c:if>
														<c:if test="${exception.status == 4}"><a href="javascript:void(0)">在途运载</a></c:if>
														<c:if test="${exception.status == 5}"><a href="javascript:void(0)">等待卸货</a></c:if>
														<c:if test="${exception.status == 6}"><a href="javascript:void(0)">等待回单</a></c:if>
													</td>
					...
					<div class="tab-pane" id="panel3">
						<div class="domain">
							<div class="tableBg">
								<a href="javascript:void(0)" class="exportBtn export"><span>导出</span></a>
								<!--<a href="javascript:void(0)" class="exportBtn"><em class="look"></em><span>查看</span></a>-->
								<a href="javascript:void(0)" class="exportBtn msgbtn msg"><span>短信告知</span></a>
							</div>
							<div class="table-responsive">
								<!--历史运单开始-->
					...
					<div class="tab-pane" id="panel4">
						<div class="domain">
							<div class="tableBg">
								<a href="javascript:void(0)" class="exportBtn rest"><span>还原</span></a>
								<a href="javascript:void(0)" class="exportBtn export"><span>导出</span></a>
								<!-- <a href="javascript:void(0)" class="exportBtn del" id=""><span>删除</span></a> -->
								<a href="javascript:void(0)" class="exportBtn msgbtn msg"><span>短信告知</span></a>
							</div>
							<div class="table-responsive">
								<!-- 项目回收站结束-->
										...
										<c:forEach var="delete" items="${deleteResult.rows}">
											<tr>
												...
												<td>${delete.projectCode}</td>
													...
													<td>
														<c:if test="${delete.status == 1}"><a href="javascript:void(0)">等待承认</a></c:if>
														<c:if test="${delete.status == 2}"><a href="javascript:void(0)">等待装车</a></c:if>
														<c:if test="${delete.status == 3}"><a href="javascript:void(0)">等待发运</a></c:if>
														<c:if test="${delete.status == 4}"><a href="javascript:void(0)">在途运载</a></c:if>
														<c:if test="${delete.status == 5}"><a href="javascript:void(0)">等待卸货</a></c:if>
														<c:if test="${delete.status == 6}"><a href="javascript:void(0)">等待回单</a></c:if>
													</td>
					...
					<div class="modal-body" id="newModalBody">
						...
						<div class="project_info">
							...
							<div class="form-inline">
								<div class="form-group">
									...
									<input type="text" id="pleaseCarNum" name="pleaseCarNum" onkeyup="advanceAmount()"/>
								</div>
								<div class="form-group">
									<label>请车类型：</label>
									<select id="select_pleaseTrain"></select>
								</div>
								<div class="form-group">
									<label>&emsp;预载重：</label>
									<input id="estimateWeight" name="estimateWeight" type="text"   />
								</div>
								<div class="form-group">
									<label>预计费用：</label>
						...
						<div class="project_info">
							...
							<div class="form-inline">
								<div class="form-group">
									<label>预付款账户：</label>
									<input id="advanceChargeAccount" name="advanceChargeAccount" type="text" unselectable="on" class="inputbg" readonly="readonly" />
								</div>
								<div class="form-group">
									<label>预付款余额：</label>
				...
				<div class="modal-content">
					...
					<div class="modal-body" id="waitUlLoadForm">
						...
						<input id="entruckInfoJson" type="hidden" name="entruckInfoJson" />
							...
							<div class="moreAdd"><img src="${pageContext.request.contextPath}/img/more.png" />添加更多装车信息</div>
						</div>
						<div class="row clearfix ">
							<div class="col-md-12 column modal_btn">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form id="carryId">
							...
							<div class="row clearfix ">
								<div class="col-md-12 column modal_btn">
									<button type="button" class="btn sureBtn" onclick="updOrderStatus(5)">确定</button>
									<button type="button" class="btn cancleBtn" data-dismiss="modal">取消</button>
								</div>
			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="row clearfix ">
							<div class="col-md-12 column modal_btn">
								<button type="button" class="btn sureBtn" onclick="updOrderStatus(7)">确定</button>
								<button type="button" class="btn cancleBtn" data-dismiss="modal">取消</button>
							</div>
						</div>
						</form>
		...
		<!-- 提示 -->
		<div class="modal fade deletRemindModal" tabindex="-1" role="dialog" id="promptIdModal" data-backdrop="static">
			<div class="modal-dialog " role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title">提示</h4>
