...
<html>
	...
	<script type="text/javascript">
			...
			function gainSelectValue(param){
				...
				$(param).next().next().next().html(fyId);
				$(param).next().next().next().next().html(fyName);
				...
				$(param).next().next().next().next().next().html(faId);
				$(param).next().next().next().next().next().next().html(faName);
			}
			...
			function saveWaitTrainInput() {
				...
				$.each(arrtbody, function(i,val){      
				    ...
				    $.each(arrTr, function(j,trVal){    
						...
						var send_weight = arrTd.eq(3).html();
						var cargo_place_id = arrTd.eq(4).html();
						var cargo_place_name = arrTd.eq(5).html();
						var cargo_site_id = arrTd.eq(6).html();
						var cargo_site_name = arrTd.eq(7).html();
						saveWait.push({
							carType : car_type,
   	                ...
   	                success: function(data) {  
   	                	...
   	                	var trainCargoList = data.data["trainCargoList"];
                            ...
                            +"</label></td><td style='display:none'></td><td>"+ (index+1) +"</td><td>"+tcList.carType+"</td><td>"+tcList.carNumber+"</td>"
	   	 					+"<td>"+tcList.sendWeight+"</td><td contentEditable='true' onkeyup='loadingValue(this)'>"+tcList.unloadWeight+"</td><td><select name='freight_guide"+index+"' class='select_freight_guide' onchange='cargoLoExchangetd(this)'></select></td><td><select name='yard_guide"+index+"' onchange='yardLoExchangetd(this)'></select></td><td name='resaonSend"+index+"'></td><td name='resaonArrive"+index+"'></td></tr>")
			...
			function loadingValue(param){
				var unloadId = $(param).prev().prev().prev().prev().prev().prev().find("input").val();
				var unloadHideId = $(param).prev().prev().prev().prev().prev();
			...
			function allocatCargoAndYard(status) {
				...
				for (var i = 0; i < trList.length; i++) {
					...
					if(tdArr.eq(1).html() == null || tdArr.eq(1).html() == ""){
						...
						var unload_weight = tdArr.eq(6).html();
				...
				//勾选再次确认，检验表格信息是否全都填写
				 ...
				 for(var i = 0; i < trList.length; i++){
					...
					var unload_weight2 = tdArr2.eq(6).html();
