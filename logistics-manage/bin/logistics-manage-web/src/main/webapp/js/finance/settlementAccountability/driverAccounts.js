...
(function($) {
  ...
  var retrieveData = function(source, data, callback) {
	  ...
	  queryCondition.cashSettleStatus=1;
  ...
  //结算页面
	$("#btnDriverAccountsSettlement").click(function(e) {
		...
		var data = tblDriverAccounts.getSelected();
		//自动填充 项目信息
		autoPaddingProjectInfo(shPackIdss,data);
			 
		$("#settlementModal").modal("show");
	});
	
	//结算页面
	$("#btnDriverAccountsSettlementStart").click(function(e) {
			var shPackIds = tblDriverAccounts.getSelections('shPackId');
			if (shPackIds.length < 1) {
				  $.fd.msg.notice("请选择至少一条信息");
				  return false;
			  }
			var cashSettleStatuss = tblDriverAccounts.getSelections('cashSettleStatus');//状态
			for (var i in cashSettleStatuss) {
				  if(cashSettleStatuss[i] != -1) {
					  $.fd.msg.notice("只能选择待开始信息");
				      return false;
				  }
			  }
			/**
		...
		ajaxOpts.data = {ids : datas};
			...
			$.confirm("开始结算","是否开始结算",{
			    success:function(){
			    var type = "put";
				var url = "api/shortPacks/settleStart/"+shPackIds;
				$.fd.ajax({
					url : url,
					type : type,
					//data : {ids : shPackIds},
	...
	//自动填充 项目信息
	function autoPaddingProjectInfo(shPackIdss,data) {
		...
		$("#chooseProjectFormId [name=receiver]").val(data.receiver);//收款人
		$("#chooseProjectFormId [name=openBank]").val(data.openBank);//开户行
		//$("#chooseProjectFormId [name=receiveOpenBankNum]").val(shPackIdss[0].bankNum);//行号
		$("#chooseProjectFormId [name=bankAccount]").val(data.bankAccount);//银行账号
		$("#chooseProjectFormId [name=suppliesExecuteName]").val(data.suppliesExecuteName);//银行账号
	}
	...
	$("#btnDriverAccountsQueryProjectId").click(function() {
	  ...
	  var data = tblDriverAccounts.getSelected();
	  var queryData = {};
	  queryData.provideCompanyId = $("#chooseProjectFormId [name=provideCompanyId]").val();
	  if(!queryData.provideCompanyId){
  ...
  function loadqueryCondition(){
    ...
    queryCondition.beginDateCheck = $("#frmDriverAccountsQuery [name=beginDate]").val();
    queryCondition.endDateCheck = $("#frmDriverAccountsQuery [name=endDate]").val();
    return queryCondition;
  }
  
  $("#btnDriverAccountsHistoryDetail").click(function(e) {
	    e.preventDefault();
	    window.open("driverAccountsHistoryDetail.do", "contentIframe");  
 });
  ...
  //
  var provideCompanyId = $("#chooseProjectFormId [name=provideCompanyId]").FdSelect2({type : 'provideCompanyId'});
  });
}(jQuery));
