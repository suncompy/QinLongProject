...
(function($) {
  ...
  // 短驳运单财务表信息一览
  var tblProjectCheck2 = $("#tblProjectCheck2").FdDataTable({
    ...
    ,{
        data : "cpackedMoney"//汽运已对账吨位
      }
    ,{
        data : "cinvoicedWeight"//汽运已开票吨位
      }
    ,{
        data : "csettledWeight"//汽运已结算吨位
      }
    ,{
        data : "rpackedMoney"//接取已对账吨位
      }
    ,{
        data : "rinvoicedWeight"//接取已开票吨位
      }
    ,{
        data : "settledWeight"//接取已结算吨位
      }
    ,{
        data : "tpackedMoney"//火运已对账吨位
      }
    ,{
        data : "tinvoicedWeight"//火运已开票吨位
      }
    ,{
        data : "tsettledWeight"//火运已结算吨位
      }
    ,{
        data : "spackedMoney"//送达已对账吨位
      }
    ,{
        data : "sinvoicedWeight"//送达已开票吨位
      }
    ,{
        data : "ssettledWeight"//送达已结算吨位
      }
    ],
    columnDefs : [ {
      targets : [ 0 ],
...
//跳转到项目明细
  $("#projectCheck1").click(function(e) {
	    e.preventDefault();
	    window.open("projectCheck.do", "contentIframe");  
 });
  ...
  $("#btnProjectCheck2ConditionQuery").click(function(e) {
    ...
    loadqueryCondition();
    // 刷新数据
    tblProjectCheck2.reload();
  });
  //导出
  $("#btnProjectCheck2Export").click(function(e) {
	    e.preventDefault();
	    //var condition = $("#searchForm").serializeJSON();
	    var conditionObj = objToUrl(loadqueryCondition());
	    window.location.href = "/logistics-manage-web/api/queryProjectCheck2/export"+conditionObj;
	  });
