$(function(){  
	...
	$(".tabbable .nav li").click(function(){
	  ...
	  $("body").on("click",".addBtn",function(){
	  	...
	  	//<tbody><tr class='newTr'><td contentEditable='true' style='text-align:center'>1</td><td><select class='form-control' name='chexing' onchange='gainSelectValue(this);'></select></td><td contentEditable='true' oninput='restrictLength(this)'></td><td contentEditable='true'><select class='form-control' name='jzx2'></select></td>\
		//<td contentEditable='true'><select class='form-control' name='jzx2'></select></td><td contentEditable='true' onblur='sumWeight(this)'></td><td style='display:none'></td><td style='display:none'></td><td style='display:none'></td><td style='display:none'></td></tr></tbody>
		...
		<table class='table loadingTable'><thead><tr class='tableTop'><th>序号</th><th>车型</th><th>车号</th><th>集装箱号1</th><th>装车载重1</th><th>集装箱号2</th><th>装车载重2</th><th>操作</th></tr></thead>\
		<tbody></tbody></table>\
   ...
   var add = (function(start){  
    ...
    var iHtml = '';  
    return function(){
      iHtml = "<tr class='newTr'><td id="+ (count++) + " contentEditable='true'>"+ count +"</td><td><select class='form-control' name='chexing' onchange='gainSelectValue(this);'></select></td><td><input type='text'  maxLength='7' onkeyup='RepNumber(this)' style='height:28px;border:1px solid #dde3ef; border-radius: 4px;'></td><td><select class='form-control' name='jzx2'></select></td>"
      +"<td><input type='text'  oninput='entruckWeightLength(this)' style='height:28px;border:1px solid #dde3ef; border-radius: 4px;'></td>"
      +"<td><select class='form-control' name='jzx2'></select></td>"		
      +"<td><input type='text'  oninput='entruckWeightLength(this)' style='height:28px;border:1px solid #dde3ef; border-radius: 4px;'></td><td style='display:none'></td><td style='display:none'></td><td style='display:none'></td><td style='display:none'></td>"
      ...
      var hiddenBeginSiteId = $("#hiddenBeginSiteId").val();
      var hidenProjectId = $("#hidenProjectId").val();
      var kind = 3;
      var containerBuffer = hidenProjectId+","+hiddenBeginSiteId+","+kind+","+""+","+1;
      $(".newTr [name=jzx2]").FdSelect2({url:'/logistics-manage-web/api/basicdata/typesByTrain/testType/ids/'+containerBuffer,type : 'trainStatus'});
   ...
   var id=$(this).parent().parent().parent().attr("id");
	...
	// 导出
	  $("#export1").click(function(e) {
	    ...
	    window.location.href = "/logistics-manage-web/trainOrder/export1"+conditionObj;
	  });
	// 导出
	  $("#export2").click(function(e) {
	    ...
	    var condition = $("#searchForm").serializeJSON();
	    var conditionObj = objToUrl(condition)+"&pageNo="+currentPage;
	    window.location.href = "/logistics-manage-web/trainOrder/export2"+conditionObj;
	  });
	  // 导出
	  $("#export3").click(function(e) {
	    ...
	    var condition = $("#searchForm").serializeJSON();
	    var conditionObj = objToUrl(condition)+"&pageNo="+currentPage;
	    window.location.href = "/logistics-manage-web/trainOrder/export3"+conditionObj;
	  });
	// 导出
	  $("#export4").click(function(e) {
	    ...
	    var condition = $("#searchForm").serializeJSON();
	    var conditionObj = objToUrl(condition)+"&pageNo="+currentPage;
	    window.location.href = "/logistics-manage-web/trainOrder/export4"+conditionObj;
	  });
