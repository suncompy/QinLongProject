				...
				if(data.status==200){
					...
					$.each(cargoPointDetailLists,function(index,cargoPointDetails){
						if(cargoPointDetails.type==0){
							$("#tPointBod").append("<tr class='newTr'><td><label class='demo--label'>\<input class='demo--checkbox moneyCheck' type='checkbox' checked ='checked' name='checkname' value='"+cargoPointDetails.type+"' >\<span class='demo--checkboxInput'></span>\</label></td><td id="+ (index++) + ">"+ index +"</td><td title='双击可编辑'>"+cargoPointDetails.pointName+"</td><td title='双击可编辑' onkeyup='placeHtml(this)'>"+cargoPointDetails.pointMin+"~"+cargoPointDetails.pointMax+"</td></tr>")
						}else{
					...
					$.each(cargoSpecificteDetailLists,function(index,cargoSpecificteDetails){
						$("#tSpecBod").append("<tr class='newTr'><td id="+ (index++) + ">"+ index +"</td><td contentEditable='true'>"+cargoSpecificteDetails.name+"</td></tr>")
					});
					$("#addCargoModal").modal();
				}
			},
									...
									<tbody id="cargoBody">
										<c:forEach items="${tbCargoMainPoint.rows}" var="it">
											<tr id="${it.id}">
												...
												<td>${it.cargoName}</td>
												<td>${it.cargoCode}</td>
												<td><c:forEach items="${it.listName}" var="spec">
													${spec.name}&nbsp;&nbsp;
												</c:forEach></td>
												<td>${it.pointName}</td>
