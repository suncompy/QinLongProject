...
<script type="text/javascript">
	function companyApply(id) {
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/driver/company/apply.do?id="+ id,
			dataType : "json",
	...
	function cancel(id,type,recordId){
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/driver/cancel.do?id="+id+"&type="+type+"&recordId="+recordId,
			dataType : "json",
			success : function(data) {
				if(data.status != 1){
					$("#pointResult").html("");
	...
	<div class="container-fluit">
		<div class="row">
			<div class="col-md-12 col-sm-12" style="min-width: 1000px;">
				<c:if test="${not empty belong_carTeam}">
					...
					<div class="anchored-fleet">
						...
						<div class="wrap fleet-info " style="overflow: hidden;">
							...
							<hr>
							<%-- 
							<c:forEach items="${belong_cpmList}" var="it" varStatus="its">
				...
				<c:if test="${not empty belong_cpmList}">
					<!-- 挂靠公司信息	  隐藏-->
					<c:forEach items="${belong_cpmList}" var="it" varStatus="its">
						...
						<div class="anchored-company">
							...
							<div class="wrap company-info" style="overflow: hidden;">
								<ul class="clearfix">
									<li><label>公司名称：</label> <span id="">${it.name}</span></li>
									<li><label>挂靠时间：</label> <span id=""> <fmt:formatDate
												value="${it.time}" pattern="yyyy.MM.dd" />
									</span></li>
									<li><label>联系方式：</label> <span id="">${it.phone}</span></li>
									<li><a  onclick="cancel(${it.id},2,${it.recordId})">取消挂靠</a></li>
								</ul>
							</div>
						</div>
					</c:forEach>
										...
										<div class="clearfix">
											...
											<div class="col-sm-2">
												<c:if test="${empty it.anchored_id}">
													<span id=""><a onclick="companyApply(${it.id})">申请挂靠</a></span>
												</c:if>
				...
				<div class="anchoredHistory">
					...
					<div class="wrap3 history-info" style="overflow: hidden;">
						<c:forEach items="${historyList}" var="it" varStatus="its">

							<c:if test="${it.type == 1}">
								<ul class="clearfix">
									<li><label>车队名称：</label> <span id="">${it.anchoredName}</span></li>
									<li><label>
											...
											<c:if test="${it.status ==1}">
												...
												value="${it.anchoredDate}" pattern="yyyy.MM.dd" />
									</span></li>
									<li><label>联系方式：</label> <span id="">${it.anchoredPhone}</span></li>
							...
							<c:if test="${it.type == 2}">
								<ul class="clearfix">
									<li><label>公司名称：</label> <span id="">${it.anchoredName}</span></li>
									<li><label><c:if test="${it.status ==2}">
												被拒时间：
											</c:if>
											<c:if test="${it.status ==1}">
												取消时间：
